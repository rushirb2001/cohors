...
/// Mount the Leptos app to the page body (called from the wasm `main`).
pub fn mount() {
    ...
    view! {
        <div class="app">
            <header class="topbar">
                ...
                // The first browser-driven *action*: fetch the whole fleet
                // server-side via POST /api/action. The server enforces the gate,
                // so on a read-only server this just surfaces "start with
                // --allow-writes" in the banner — proof the write seam reaches the
                // web surface (it no longer lives only in the binary).
                <button
                    class="ghost fetch-all"
                    on:click=move |_| {
                        spawn_local(async move {
                            let body = serde_json::json!({
                                "verb": "fetch", "selector": { "all": true }, "confirm": true
                            });
                            let msg = match api::post_action(body).await {
                                Ok(v) => v
                                    .get("error")
                                    .and_then(|e| e.as_str())
                                    .map(str::to_string)
                                    .unwrap_or_else(|| {
                                        format!(
                                            "fetched {} repo(s)",
                                            v.get("targets").and_then(|t| t.as_u64()).unwrap_or(0)
                                        )
                                    }),
                                Err(e) => e,
                            };
...
fn changes_cell(s: &RepoSnapshot) -> impl IntoView + use<> {
    ...
    if total == 0 {
        ...
        let color = if w.modified > 0 || w.untracked > 0 {
            "modified"
        } else {
            "staged"
        };
        ...
        let prs = (!r.prs.is_empty()).then(|| {
            let rows = r
                ...
                .map(|p| {
                    let draft = p
                        .draft
                        .then(|| view! { <span class="badge">"draft"</span> });
                    view! {
                        <li>
